...
from gym.envs.mujoco.HumanKickDefend import HumanKickDefendEnv
from gym.spaces import Box, Discrete, Dict
import numpy as np
from os import path
from ray.rllib.env.multi_agent_env import MultiAgentEnv
from visualize.plot_heatmap import HumanKickDefend_friction_sweep, HumanKickDefend_mass_sweep
from copy import deepcopy
class AdvMAHumanKickDefend(HumanKickDefendEnv, MultiAgentEnv):
    def __init__(self, config):
        ...
        super(AdvMAHumanKickDefend, self).__init__()
    ...
    def extreme_randomize_domain(self):
        ...
        self.friction_coef = np.random.choice(HumanKickDefend_friction_sweep, num_geoms)[:, np.newaxis]
        self.mass_coef = np.random.choice(HumanKickDefend_mass_sweep, num_masses)
    ...
    def randomize_domain(self):
        self.friction_coef = np.random.choice(HumanKickDefend_friction_sweep)
        self.mass_coef = np.random.choice(HumanKickDefend_mass_sweep)
    ...
    def step(self, actions):
        ...
        if isinstance(actions, dict):
            # the HumanKickDefend action before any adversary modifies it
            obs_HumanKickDefend_action = actions['agent']
            HumanKickDefend_action = actions['agent']
            ...
            if self.adversary_range > 0 and 'adversary{}'.format(self.curr_adversary) in actions.keys():
                if self.adv_all_actions:
                    ...
                    HumanKickDefend_action += adv_action
                    # apply clipping to HumanKickDefend action
                    if self.clip_actions:
                        HumanKickDefend_action = np.clip(obs_HumanKickDefend_action, a_min=self.action_space.low, a_max=self.action_space.high)
                else:
                    adv_action = actions['adversary{}'.format(self.curr_adversary)] * self.strengths[self.curr_adversary]
                    self._adv_to_xfrc(adv_action)
        else:
            assert actions in self.action_space
            obs_HumanKickDefend_action = actions
            HumanKickDefend_action = actions
        ...
        self.do_simulation(HumanKickDefend_action, self.frame_skip)
        posafter, height, ang = self.sim.data.qpos[0:3]
        alive_bonus = 1.0
        reward = (posafter - posbefore) / self.dt
        reward += alive_bonus
        reward -= 1e-3 * np.square(HumanKickDefend_action).sum()
        s = self.state_vector()
        ...
        if self.concat_actions:
            self.update_observed_obs(np.concatenate((ob, obs_HumanKickDefend_action)))
        else:
...
def HumanKickDefend_env_creator(env_config):
    env = AdvMAHumanKickDefend(env_config)
    return env
